...
module.exports = async (client, message) => {
  ...
  if (`${message.mentions.members.first()}` === `<@${client.user.id}>`) message.channel.send(`Mon prefix est: \`${PREFIX}\``)
  ...
  if (command.help.raw && !args.length) {
  ...
  if (command.help.block) return message.channel.send(`Cette commande est actuellement bloqué merci d'attendre que mon devellopeurs la débloque`)

  command.run(client, message, args, dbUser);
}